    ...
    def readDatabases(self,databases,message):
        ...
        if len(potentials)>0: #if found
            ...
            for i in potentials: #get largest
                ...
                if l>0.75: #really big chance
                    potentials3.append(i[0])
            ...
            if l>0.9: #if the conversation is really relevant
                print("*")
                c=0
                val=choice(potentials3) #return one of largest
                while c<len(potentials3)-1 and val in self.convo: #get one which has not been used
                    del potentials3[potentials3.index(val)] #remove
                    val=choice(potentials3) #return one of largest
                    c+=1
                return val
            print("~")
